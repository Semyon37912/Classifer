...
2.Преобразовал категориальные признаки в числовые./n
...
В Naive Bayes каждый объект описывается набором признаков, и каждый признак является независимым от других признаков. Это предположение
называется "наивным" (naive), поскольку в реальности многие признаки могут быть зависимыми.
...
### Precision - 
это метрика для оценки качества модели машинного обучения, которая показывает, как много объектов, отнесенных к определенному классу
моделью, действительно являются членами этого класса.

Precision вычисляется как отношение числа истинных положительных (True Positive, TP) классификаций к общему числу объектов, которые были
классифицированы как положительные (True Positive + False Positive, TP+FP).

Precision = TP / (TP + FP)

### Recall -
это метрика для оценки качества модели машинного обучения, которая показывает, как много объектов определенного класса модель нашла из
общего числа объектов этого класса.

Recall вычисляется как отношение числа истинных положительных (True Positive, TP) классификаций к общему числу объектов, которые
действительно принадлежат этому классу (True Positive + False Negative, TP+FN).

Recall = TP / (TP + FN)

### F1 score -
это гармоническое среднее между Precision и Recall. F1 score - это метрика для оценки качества модели машинного обучения, которая
учитывает как точность, так и полноту модели.
...
### Accuracy -
это метрика, используемая для оценки производительности модели машинного обучения. Она показывает, какая доля правильных предсказаний
сделана моделью на тестовом наборе данных.

Для вычисления Accuracy мы сначала определяем общее число правильных предсказаний, сделанных моделью на тестовом наборе данных. Затем мы
делим это число на общее число примеров в тестовом наборе данных. Формально это можно записать следующим образом:

Accuracy = (Количество правильных предсказаний) / (Общее число предсказаний)
...
6.Использование ранней остановки - ранняя остановка является методом регуляризации, который заключается в прекращении обучения модели,
когда ошибка на тестовой выборке начинает увеличиваться. Это позволяет остановить обучение модели на ранней стадии, когда она начинает
переобучаться.
